...
(async function () {
  ...
  logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "cục cưng của nmai" : global.config.BOTNAME}`, "[ BOT INFO ]");
  ...
  const handleUnsend = require("./handle/handleUnsend")({ api, models, Users, Threads, Currencies });
  ...
  const checkTime = (time) => new Promise((resolve) => {
    ...
    if (time[2] < 2023) resolve("Bạn sống ở kỷ nguyên nào thế?");
