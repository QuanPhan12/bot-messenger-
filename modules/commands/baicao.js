...
module.exports.handleEvent = async ({ Currencies, event, api, Users }) => {
  ...
  const fs = require ("fs-extra");
    ...
    if (body.indexOf("Đổi bài") == 0 || body.indexOf("đổi bài")   == 0) {
        ...
        if (player.ready == true) return api.sendMessage("『 🌸 』➝ 𝐁𝐚̣𝐧 𝐯𝐮̛̀𝐚 sẵn sàng, 𝐛𝐚̣𝐧 𝐤𝐡𝐨̂𝐧𝐠 𝐭𝐡𝐞̂̉ 𝐭𝐡𝐚𝐲 𝐛𝐚̀𝐢 !", threadID, messageID);
     ...
     switch (args[0]) {
      ...
      await Currencies.decreaseMoney(event.senderID, Number(args[1]));
        ...
        case "-j": {
            ...
            if (values.player.find(item => item.id == senderID)) return api.sendMessage("[🃏] 𝐁𝐚̣𝐧 đã 𝐭𝐡𝐚𝐦 𝐠𝐢𝐚 𝐭𝐮̛̀ 𝐭𝐫𝐮̛𝐨̛́𝐜 𝐫𝐨̂̀𝐢!", threadID, messageID);
        ...
        case "-s": {
            if (!values) return api.sendMessage("[🃏] 𝐇𝐢𝐞̣̂𝐧 𝐭𝐚̣𝐢 𝐜𝐡𝐮̛𝐚 𝐜𝐨́ 𝐛𝐚̀𝐧 𝐛𝐚̀𝐢 𝐜𝐚̀𝐨 𝐧𝐚̀𝐨, 𝐛𝐚̣𝐧 𝐜𝐨́ 𝐭𝐡𝐞̂̉ 𝐭𝐚̣𝐨 𝐛𝐚̆̀𝐧𝐠 𝐜𝐚́𝐜𝐡 𝐬𝐮̛̉ 𝐝𝐮̣𝐧𝐠 !𝐛𝐚𝐢𝐜𝐚𝐨 𝐜𝐫𝐞𝐚𝐭𝐞", threadID, messageID);
            if (values.author !== senderID) return api.sendMessage("[🃏] 𝐁𝐚̣𝐧 𝐤𝐡𝐨̂𝐧𝐠 𝐩𝐡𝐚̉𝐢 𝐥𝐚̀ 𝐧𝐡𝐚̀ 𝐜𝐚́𝐢 𝐤𝐡𝐨̂𝐧𝐠 𝐭𝐡𝐞̂̉ 𝐛𝐚̆́𝐭 đầu", threadID, messageID);
