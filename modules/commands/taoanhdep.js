...
module.exports.run = async function({ api, args, event, permssion }) {
 return api.sendMessage("『 🌸 』➝ Phản Hồi Tin Nhắn Này Để Nhập Tên Bạn Nam", event.threadID, (err, info) => {
...
if(!fs.existsSync(__dirname+'/tad/SVN-Holidays.ttf')) { 
  let getfont2 = (await axios.get(`https://github.com/hanakuUwU/font/raw/main/SVN-Holidays.ttf`, { responseType: "arraybuffer" })).data;
   ...
   let path = __dirname + `/tad/avatar_1.png`;
    ...
    return api.sendMessage(`『 🌸 』➝ Bạn đã chọn tên bạn nam là ${event.body}, phản hồi tin nhắn này để nhập năm sinh hoặc chiều cao của bạn nam :>`, event.threadID, (err, info) => {
    ...
    api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`『 🌸 』➝ Bạn đã nhập năm sinh bạn nam là ${event.body}, phảm hồi tin nhắn này để nhập tên của bạn nữ`, event.threadID, (err, info) => {
      if(err) return api.sendMessage(`『 🌸 』➝ Lỗi bất định`, event.threadID)
      return global.client.handleReply.push({
        step: 3,
        name: this.config.name,
     ...
     api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`『 🌸 』➝ Bạn chọn tên bạn nữ là ${event.body}, phản hồi tin nhắn này để nhập năm sinh hoặc chiều cao của bạn nữ`, event.threadID, (err, info) => {
      if(err) return api.sendMessage(`『 🌸 』➝ Lỗi bất định`, event.threadID)
      return global.client.handleReply.push({
        step: 4,
        name: this.config.name,
