...
if (!fs.existsSync(__dirname+'/cache'))fs.mkdirSync(__dirname+'/bot');
...
let data = {};
let path = __dirname+'/cache/autorep.json'
...
exports.handleReply = function(o) {
  ...
  switch (_.type) {
    case '1': {
      if (!data[tid][key])return send(`[🔰] ➜ Từ khóa autorep không có trong dữ liệu!`);
