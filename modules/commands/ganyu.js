...
module.exports.handleEvent = async ({ api, event, Threads }) => {
  if (event.body.indexOf("Ganyu")==0 || (event.body.indexOf("ganyu")==0)) {
...
const fs = global.nodemodule["fs-extra"];
    var link = [
     ...
     var callback = () => api.sendMessage({body:``,attachment: fs.createReadStream(__dirname + "/noprefix/ganyu.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/noprefix/ganyu.jpg"), event.messageID);  
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname+"/noprefix/ganyu.jpg")).on("close",() => callback());
}
                                                                                                         }
