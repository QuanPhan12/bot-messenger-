module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Vihoo",
  description: "Random ảnh nhóm nhạc Blackpink",
...
module.exports.handleEvent = async ({ api, event, Threads }) => {
  if (event.body.indexOf("Blackpink")==0 || (event.body.indexOf("blackpink")==0)) {
...
const fs = global.nodemodule["fs-extra"];
    var link = [
     ...
     var callback = () => api.sendMessage({body:``,attachment: fs.createReadStream(__dirname + "/noprefix/blackpink.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/noprefix/blackpink.jpg"), event.messageID);  
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname+"/noprefix/blackpink.jpg")).on("close",() => callback());
}
                                                                                                         }
