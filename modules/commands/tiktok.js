  ...
  switch (handleReply.type) {
    ...
    case 'search':
      ...
      axios.get(videoInfo[index].nowatermark, { responseType: "stream" }).then(res => {
        ...
        res.data.on("end", () => {
          api.sendMessage({ body: `====== TIKTOK ======\n\n🌍 Quốc gia: ${region}\n📝 Tiêu đề: ${title}\n✏️ Tên kênh: ${nickname}\n🌸 ID người dùng: ${unique_id}\n❤️ Lượt tim: ${digg_count}\n💬 Tổng bình luận: ${comment_count}\n👀 Lượt xem: ${play_count}\n📎 Lượt chia sẻ: ${share_count}\n⚡️ Lượt tải: ${download_count}\n⏰️ Thời gian: ${duration} giây`, attachment: fs.createReadStream(__dirname + "/cache/tiktok.mp4") }, threadID, () => fs.unlinkSync(__dirname + "/cache/tiktok.mp4"), messageID);
        });
...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (!args[0]) return api.sendMessage({body:"[ Hướng Dẫn Sử Dụng ]\n━━━━━━━━━━━━━━━━━━━━\n\n?tiktok video (link): Tải video ❤️\n\n?tiktok music (link): Tải âm thanh của video 🥰\n\n?tiktok search (từ khóa): Tìm kiếm video thông qua từ khoá 🔎\n", attachment: (minhtien)},event.threadID);
  ...
  switch (type.toLowerCase()) {
    ...
    case "info":
      ...
      try {
        axios.get(encodeURI(`https://sumiproject.io.vn/tiktok?info=${keyword}`)).then(async (res) => {
  ...
  fs.writeFileSync(path, Buffer.from(getimg, "utf-8"));
    ...
    case '-s':
      ...
      axios.get(`https://sumiproject.io.vn/tiktok?search=${encodeURI(search)}`).then(async res => {
    ...
    case "video":
      try {   
        const res = await axios.get(`https://sumiproject.io.vn/tiktok?video=${keyword}`);
    ...
    case "-tr":
    case "trending":
      axios.get(`https://thenamk3.net/api/tiktok.json?trending=VN&apikey=qasFbExc`).then(response_api => {
