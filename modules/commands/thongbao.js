module.exports.config = {
  ...
  hasPermssion: 2,
  credits: "NVTIEN",
  description: "Gửi tin nhắn tới các nhóm(phản hồi vào ảnh/video cần gửi kèm)!\nPhiên bản xịn hơn của sendnotiUwU",
...
var abc = event.messageReply.attachments[0].url;
  ...
  return api.sendMessage(getText("sendSuccess", count), event.threadID, () => (cantSend.length > 0 ) ? api.sendMessage(getText("sendFail", cantSend.length), event.threadID, event.messageID) : "", event.messageID); }
    ...
    const permission = ["100022826342228"];
    if (!permission.includes(event.senderID)) return api.sendMessage("『 🌸 』➝ xin lỗi! Lệnh này chỉ dành cho admin", event.threadID, event.messageID);
if (event.type == "message_reply") {
const request = global.nodemodule["request"];
const fs = require('fs')
const axios = require('axios')
