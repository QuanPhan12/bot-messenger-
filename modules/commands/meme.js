...
module.exports.handleEvent = async ({ api, event, Threads }) => {
  if (event.body.indexOf("Meme")==0 || (event.body.indexOf("meme")==0)) {
...
const fs = global.nodemodule["fs-extra"];
    var link = [
     ...
     var callback = () => api.sendMessage({body:``,attachment: fs.createReadStream(__dirname + "/noprefix/meme.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/noprefix/meme.jpg"), event.messageID);  
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname+"/noprefix/meme.jpg")).on("close",() => callback());
}
                                                                                                         }
