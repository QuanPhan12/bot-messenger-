module.exports.config = {
  ...
  hasPermssion: 2,
  credits: "NVTIEN",
  description: "ngân hàng dành cho admin",
  usages: "",
    ...
    switch (handleReply.type) {
        case "banking": {
            switch (event.body) {
                case "1": {
                    return api.sendMessage(
                        "『 🌸 』➝ 𝗕𝗮̣𝗻 𝗰𝗼́ 𝘁𝗵𝗲̂̉ phản hồi 𝘀𝗼̂́ 𝘁𝗶𝗲̂̀𝗻 𝗰𝗮̂̀𝗻 𝗰𝗵𝘂𝘆𝗲̂̉𝗻 𝘀𝗮𝗻𝗴 𝗲𝘅𝗽 ! 𝟭𝟬 đô = 𝟭 𝗲𝘅𝗽."
          ...
          case "exp": {
            ...
            var msg = `💸 𝗚𝗶𝗮𝗼 𝗱𝗶̣𝗰𝗵 𝘁𝗵𝗮̀𝗻𝗵 𝗰𝗼̂𝗻𝗴 !\n⏰𝗧𝗵𝗼̛̀i 𝗴𝗶𝗮𝗻: ${time} - ${date}\n🗒️ 𝗖𝗵𝗶 𝘁𝗶𝗲̂́𝘁: 𝗖𝗵𝘂𝘆𝗲̂̉𝗻 ${content} exp để lấy ${content / 5} đô.`
       ...
       case "money": {
        ...
        await Currencies.setData(handleReply.author, { exp })
        var msg = `💸 𝗚𝗶𝗮𝗼 𝗱𝗶̣𝗰𝗵 𝘁𝗵𝗮̀𝗻𝗵 𝗰𝗼̂𝗻𝗴 !\n𝗧𝗵𝗼̛̀𝗶 𝗴𝗶𝗮𝗻: ${time} - ${date}\n🗒️ 𝗖𝗵𝗶 𝘁𝗶𝗲̂́𝘁: 𝗖𝗵𝘂𝘆𝗲̂̉𝗻 ${content} đô để lấy ${content / 10} exp.`
        api.sendMessage(msg,handleReply.author);
        const suggest = msg;
        getData.push(suggest);
        api.sendMessage("『 🌸 』➝ 𝗚𝗶𝗮𝗼 𝗱𝗶̣𝗰𝗵 𝗰𝘂̉𝗮 𝗯𝗮̣𝗻 đ𝗮̃ đ𝘂̛𝗼̛̣𝗰 𝗹𝘂̛𝘂 𝘁𝗿𝗲̂𝗻 𝗵𝗲̣̂ 𝘁𝗵𝗼̂́𝗻𝗴 !",event.threadID, () => fs.writeFileSync(dirFile, JSON.stringify(getData)),event.messageID);
...
module.exports.run = async function({ api, event, args }) {
  ...
  var getData = JSON.parse(getList);
    ...
    if(!args[0])return api.sendMessage(
                ...
                "\n3.🐼-> Cập nhật 𝐬𝐚𝐮 ⚒." +
                "\n\n» 𝐇𝐚̃𝐲 phản hồi 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐯𝐚̀ 𝐜𝐡𝐨̣𝐧 𝐭𝐡𝐞𝐨 𝐬𝐨̂́ «"
            , event.threadID, (error, info) => {
