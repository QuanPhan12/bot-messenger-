...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (type == 'menu') {
    if (["01", "1", "02", "2"].includes(args[0])) {
      ...
      reply(`『 🌸 』➝ Phản hồi tin nhắn này kèm ảnh hoặc liên kết ảnh muốn đổi thành avatar bot`, (err, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    else return reply(`『 🌸 』➝ Vui lòng nhập liên kết hình ảnh hợp lệ hoặc phản hồi tin nhắn kèm một ảnh muốn đặt làm avatar cho bot`, (err, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    if (!body) return reply('『 🌸 』➝ Vui lòng nhập nội dung bạn muốn bình luận cho bài viết', (e, info) => {
 global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    if (!body) return reply(`『 🌸 』➝ Vui lòng nhập id bài viết bạn muốn sự phản ứng lại`, (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`『 🌸 』➝ Nhập cảm xúc bạn muốn sự phản ứng lại cho ${listID.length} bài viết (unlike/like/love/heart/haha/wow/sad/angry)`, (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`『 🌸 』➝ Đã thả cảm xúc ${feeling} cho ${success.length} bài viết thành công${failed.length > 0 ? `『 🌸 』➝ Sự phản ứng lại thất bại ${failed.length} bài viết, postID: ${failed.join(" ")}` : ''}`);
  }
    ...
    reply(`『 🌸 』➝ Đã ${type == 'chấp nhận yêu cầu kết bạn' ? 'chấp nhận' : 'xóa'} lời mời kết bạn thành công của ${success.length} id${failed.length > 0 ? `\n『 🌸 』➝ Thất bại với ${failed.length} id: ${failed.join(" ")}` : ""}`);
  }
