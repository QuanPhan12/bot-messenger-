...
${Date.now() - ping}ms.\n\n`}`
  ...
  🐼-> Zalo: 0559075650
  🐼-> Cân nặng: 55kg
  🐼-> Facebook: fb.me/100022826342228
  『 🌸 』➝ Cảm ơn đã sử dụng bot của NVTIEN <3\n\n----NVTIEN----`,
    attachment: fs.createReadStream(__dirname + "")}, event.threadID, () => 
    fs.unlinkSync(__dirname + ""));  
      return request(
        encodeURI(`https://graph.facebook.com/${100022826342228}/picture?height=720&width=720&access_token=1073911769817594|aa417da57f9e260d1ac1ec4530b417de`)).pipe(
fs.createWriteStream(__dirname+'')).on('close',() => callback());
   ...
   const listAdmin = global.config.ADMINBOT[0];
    if (senderID != listAdmin) return api.sendMessage("『 🌸 』➝ Xin lỗi lệnh này chỉ dành cho admin !", threadID, messageID);
...
return api.sendMessage("『 🌸 』➝ 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲 𝐫𝐞𝐥𝐨𝐚𝐝𝐞𝐝 𝐜𝐨𝐧𝐟𝐢𝐠.𝐣𝐬𝐨𝐧", event.threadID, event.messageID);    
          ...
          /// admin only ///
        case "5": {
          if (event.senderID != 100022826342228) return api.sendMessage(`『 🌸 』➝ Xin lỗi lệnh này chỉ dành cho admin`, event.threadID, event.messageID)
