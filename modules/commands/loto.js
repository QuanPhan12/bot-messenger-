...
module.exports.languages = {
  "vi": {
    ...
    "playersNotEnough": "『 🌸 』➝ Không đủ người chơi, tối thiểu: 2 người, hiện có: 1 người",
...
module.exports.onLoad = async () => {
  await require('axios').get("https://raw.githubusercontent.com/RFS-ADRENO/mirai-modules/main/version.json").then(res => {
    if (res.data["loto_x024"] != this.config.version) console.log("『 🌸 』➝ LOTO ĐÃ CÓ PHIÊN BẢN MỚI, LIÊN HỆ NVTIEN ĐỂ ĐƯỢC CẬP NHẬT");
  });
          ...
          return new Promise(async (resolve, reject) => {
            sendTC(getText("testInbox"), async () => {
              for (let p in global.client.loto[threadID].data) {
                ...
                api.sendMessage("『 🌸 』➝ Kiểm tra...", p, async (err) => {
                  if (err) {
                    check = true;
                    console.log(err);
