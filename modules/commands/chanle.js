...
module.exports.config = {
    ...
    hasPermssion: 0,
    creditss: `NVTIEN`,
    description: `Chẵn lẻ như tài xỉu`,
    commandCategory: `GAME`,
    usages: `Bấm /chanle [Chẵn/Lẻ] [ Số Tiền ]`,
    cooldowns: 5
};
function isEven(n) {
    return n % 2 == 0;
...
module.exports.run = async ({ api, event, args, Currencies }) => {
    ...
    switch(isOdd(parseInt(rdm))) {
                ...
                case false: answer = "chẵn";
                    break;
                default: return api.sendMessage("[Lucky🌟] => Lỗi !",event.threadID);
            }
            if (!args[0]) return api.sendMessage("[Lucky🌟] => Bấm !chanle [Chẵn/Lẻ] [ Số Tiền ]",event.threadID);
     var option;
