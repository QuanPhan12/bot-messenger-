    ...
    switch (args[0]) {
        ...
        case "-r": { 
            if (!existsSync(path)) {
                ...
                return api.sendMessage("💮====[POKEMON]====💮\n⚔️ Đăng kí nuôi pokemon thành công ⚔️", threadID, messageID);
            }
            else return api.sendMessage("💮====[POKEMON]====💮\n⚔️ Bạn đã có trong cơ sở dữ liệu ⚔️", threadID, messageID);
            break;
        }
        case 'info':
        case '-i': {
        ...
        case '-l': {
            ...
            for (let i of listPoke) {
                index++
                var msg = `🔍 ID: ${index}\n🕵️‍♀️ NamePoke: ${i.name} - ${i.coins}$\n🧡 HP: ${i.power.HP}\n🗡 Attack: ${i.power.Attack}\n🛡 Defense: ${i.power.Defense}\n⚡️ Speed: ${i.power.Speed}\n\n`;
                list.push(msg)
            }
            var page = 1;
...
module.exports.handleReply = async ({ event, api, Currencies, handleReply, Users }) => {
    ...
    switch (handleReply.type) {
        ...
        case 'giftcode': {
            ...
            var msg = {body: `🧡 GIFTCODE TÂN THỦ!\n☑️ Bạn mở trúng ${rd.name}.\n『 🌸 』➝ Type: ${rd.type}\n🔍 Chỉ số: \n🧡 HP: ${rd.power.HP}\n🗡 Attack: ${rd.power.Attack}\n🛡 Defense: ${rd.power.Defense}\n⚡️ Speed: ${rd.power.Speed}\n💰 Coins: 0$\n☑️ Đã thêm pokemon vào kho đồ của bạn!`, attachment: image}
        ...
        case 'pokemon': {
            ...
            var image = [], 
                ...
                name = '🕵️‍♀️ Tên: ' + listPoke[parseInt(body) -1].name,
                ...
                Speed = '⚡️ Tốc độ: ' + listPoke[parseInt(body) -1].power.Speed,
        ...
        case 'shop': {
            if (isNaN(body)) return api.sendMessage("💮====[POKEMON]====💮\n━━━━━━━━━━━━━━━\m『 🌸 』➝ Lựa chọn của bạn không phải là một con số!", threadID, messageID);
        ...
        case 'choose_foods': {
            ...
            writeFileSync(path, JSON.stringify(user, null, 2));
            api.unsendMessage(handleReply.messageID)
            return api.sendMessage('☑️ Mua thành công: ' + choose + ` - ${(parseInt(body) * 1000)}$\n🧡 HP: ${parseInt(body) * 4}\n🗡 Attack: ${parseInt(body) * 4}\n🛡 Defense: ${parseInt(body) * 4}\n⚡️ Speed: ${parseInt(body) * 4}\n🐳Type: ${handleReply.typePoke}`, threadID, messageID)
        }
        case 'petFoods': {
            const pathA = require("path");
            const path = pathA.join(__dirname, 'pokemon', 'datauser', `${senderID}.json`);
        ...
        case 'choose_box': {
            ...
            if(body == 1) {
                ...
                var msg = '☑️ Mua thành công rương thường (5000$)\n👉 Thả cảm xúc "👍" vào để mở nó'
                    api.unsendMessage(handleReply.messageID)
            ...
            if(body == 2) {
                ...
                var msg = '☑️ Mua thành công rương tuyệt phẩm (10000$)\n👉 Thả cảm xúc "👍" vào để mở nó'
                    api.unsendMessage(handleReply.messageID)
            ...
            if(body == 3) {
                ...
                var msg = '☑️ Mua thành công rương VIP (20000$)\n👉 Thả cảm xúc "👍" vào để mở nó'
                    api.unsendMessage(handleReply.messageID)
            ...
            if(body == 4) {
                ...
                var msg = '☑️ Mua thành công rương FREE 3 ngày/1 lần (0$)\n👉 Thả cảm xúc "👍" vào để mở nó'
                    api.unsendMessage(handleReply.messageID)
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout( async function () {
                            ...
                            setTimeout( async function () {
                                ...
                                writeFile(path, buf)
                                setTimeout( function () {
                                    api.sendMessage({body: '⚔️ Kết quả trận đấu có sau trong giây lát!', attachment: createReadStream(path)}, threadID)
                                    ...
                                    setTimeout( async function () {
                                        ...
                                        if(TB_1 > TB_2) {
                                            ...
                                            var msg = {body: `🎉 ${index_1.choose.msg.name} của người chơi ${name_1} đã chiến thắnggg!\n☑️ Tất cả chỉ số cơ bản của pokemon thắng này được tăng 5%, ngược lại thì sẽ bị trừ 5%!`, attachment: imageee}
                                            ...
                                            var lose = user_1.solo
                                                ...
                                                writeFileSync(path_1, JSON.stringify(user_1, null, 2));
                                                writeFileSync(path_2, JSON.stringify(user_2, null, 2));
                                            var msg = {body: `🎉 ${index_2.choose.msg.name} của người chơi ${name_2} đã chiến thắnggg!\n☑️ Tất cả chỉ số cơ bản của pokemon này được tăng 5%, ngược lại thì sẽ bị trừ 5%!`, attachment: imageee}
                                            return api.sendMessage(msg, threadID);
                                        } 
                                        else {
                                            var user_1 = require('./pokemon/datauser/' + `${index_1.userID}.json`);
...
module.exports.handleReaction = async ({ api, event, handleReaction, Currencies }) => {
    ...
    switch (handleReaction.type) {
        case 'buyPokemon': {
            ...
            var msg = {body: `☑️ Mua thành công: ${index.name} - ${index.coins}$\n🔍 Chỉ số:\n🧡 HP: ${index.power.HP}\n🗡 Attack: ${index.power.Attack}\n🛡 Defense: ${index.power.Defense}\n⚡️ Speed: ${index.power.Speed}\n📌 Skill: ${skillS.replace(/,\s*$/, "")}`, attachment: image}
            return api.sendMessage(msg, threadID, messageID);
        }
        case 'infoPoke': {
            var skill = [];
        ...
        case 'openBox': {
            ...
            writeFileSync( __dirname + "/pokemon/cache/pokemon.png", Buffer.from(pokemon, "utf-8") );
            image.push(createReadStream(__dirname + "/pokemon/cache/pokemon.png"));
            var msg = {body: `🎉 Xin chúc mừng!\n☑️ Bạn mở trúng ${rd.name}.\n🐼 Type: ${rd.type}\n🔍 Chỉ số: \n🧡 HP: ${(rd.power.HP* handleReaction.tpy).toFixed(1)}\n🗡 Attack: ${(rd.power.Attack* handleReaction.tpy).toFixed(1)}\n🛡 Defense: ${(rd.power.Defense* handleReaction.tpy).toFixed(1)}\n⚡️ Speed: ${(rd.power.Speed* handleReaction.tpy).toFixed(1)}\n📌 Skill: ${skillS.replace(/,\s*$/, "")}\n💰 Coins: ${handleReaction.money}$\n☑️ Đã thêm pokemon vào kho đồ của bạn!`, attachment: image}
            return api.sendMessage(msg, threadID, messageID);
        }
    }
}