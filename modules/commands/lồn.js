...
const fs = global.nodemodule["fs-extra"];
    var link = [
     ...
     var callback = () => api.sendMessage({body:``,attachment: fs.createReadStream(__dirname + "/noprefix/lon.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/noprefix/lon.jpg"), event.messageID);  
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname+"/noprefix/lon.jpg")).on("close",() => callback());
}
                                                                                                         }
