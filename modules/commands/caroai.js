...
module.exports.handleReply = async function({ event, api, handleReply }) {
  ...
  if(!isNaN(number) && number > 0 && number < 10) {
    ...
    var msg = lmao !== "" ? lmao : temp == undefined ? "『 🌸 』➝ Phản hồi số ô để đánh dấu" : temp;
...
module.exports.run = async function ({ event, api, args }) {
  ...
  if (args[0].toLowerCase() == "continue") {
    ...
    return api.sendMessage({ body: "『 🌸 』➝ Phản hồi số ô để đánh dấu", attachment: await displayBoard(data)}, threadID, (error, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        author: senderID,
        messageID: info.messageID
  ...
  if (!data.gameOn) {
    ...
    if (abc == "o") {
      ...
      api.sendMessage({ body: "『 🌸 』➝ Bạn đi trước!\n『 🌸 』➝ Phản hồi số ô để đánh dấu", attachment: await displayBoard(newData)}, threadID, (error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          author: senderID,
          messageID: info.messageID
    ...
    if (abc == "x") {
      ...
      AIStart(newData);
      api.sendMessage({ body: "『 🌸 』➝ AI đi trước!\n『 🌸 』➝ Phản hồi số ô để đánh dấu", attachment: await displayBoard(data)}, threadID,(error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          author: senderID,
          messageID: info.messageID
